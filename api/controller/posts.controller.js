...
module.exports = {
  ...
  getComments: async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ message: "get comments success", comments: comment });
    } catch ({ message = "Invalid request" }) {
      res.status(400).json({ message });
    }
  },
  ...
  postComment: async (req, res) => {
    ...
    try {
      ...
      await Comment.find().then((doc) => {
        return res
          .status(200)
          .json({ message: "Add comment success", comments: doc });
      });
    } catch ({ message = "Invalid request" }) {
      res.status(400).json({ message });
    }
  ...
  getPostInstagtam: async (req, res) => {
    try {
      await Post.find().then((doc) =>
        res.status(200).json({ message: "get posts success", posts: doc })
      );
    } catch ({ message = "Invalid request" }) {
      res.status(400).json({ message });
    }
